...
macro d_xa(A)  esc(:( $A[x+1,y]-$A[x,y] )) end
macro d_ya(A)  esc(:( $A[x,y+1]-$A[x,y] )) end

function compute_flux!(qDx, qDy, Pf, k_ηf_dx, k_ηf_dy, _1_θ_dτ)
    nx, ny = size(Pf)
    Threads.@threads for y in 1:ny
        for x in 1:nx-1
            qDx[x+1,y] -= (qDx[x+1,y] + k_ηf_dx * @d_xa(Pf)) * _1_θ_dτ
        end
    end

    Threads.@threads for y in 1:ny-1
        for x in 1:nx
            qDy[x,y+1] -= (qDy[x,y+1] + k_ηf_dy * @d_ya(Pf)) * _1_θ_dτ
        end
    end
...
function update_Pf!(Pf,qDx,qDy,_dx,_dy,_β_dτ)
    ...
    Threads.@threads for y in 1:ny
        for x in 1:nx
            Pf[x,y] -= _β_dτ * (_dx * @d_xa(qDx) + _dy * @d_ya(qDy))
        end
    end
